...
本质就是一块基于内存的数据结构，由一大堆缓存页和数据块组成，然后加上了各种链表(free、flush、lru)来辅助运行，核心是通过使用内存而不是直接使用磁盘来提高访问速度。Java系统对数据库执行增删改查请求主要就是对这个内存组件中的缓存数据进行访问的
...
通常我们认为mysql数据库中有一个一个的表，一个表有很多字段，一个表中有很多条数据，每行数据都有自己的字段值，mysql提供给用户的数据模型是表+字段+行记录的概念。但实际上在mysql底层物理结构是以数据页为单位的，数据页是mysql中抽象出来的数据单位，它把很多行数据放在了一个数据页里。也就是说磁盘文件中有很多数据页，每格数据页存放了很多行数据，所以bufferPool中存放的是一个一个的数据页。
