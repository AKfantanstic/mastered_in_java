...
本质就是一块基于内存的数据结构，由一大堆缓存页和描述数据块组成，然后加上了各种链表(free、flush、lru)来辅助运行。核心是通过使用内存而不是直接使用磁盘来提高访问速度，Java系统对数据库执行增删改查请求主要就是对这个内存组件中的缓存数据进行访问的
...
表、字段、行是mysql的逻辑概念，而底层物理存储是按数据页存储的，将多行数据放在一个数据页里，然后每个磁盘文件中存储很多个数据页。当更新数据时，数据库先找到这行数据所在的数据页，然后从磁盘文件里把这行数据所在的数据页直接加载到bufferPool里，所以bufferPool中存放的是一个一个的数据页，与磁盘中存储的数据页是一一对应的关系。

bufferPool中缓存页的描述数据：当前缓存数据页所属的表空间，数据页的编号，及其他信息
