...
## JVM在什么情况下会加载一个类？(类的加载时机)

jar包或者war包中会指定一个类的main方法作为运行入口，当jvm启动后，先由类加载器把这个主类加载到内存中，然后开始执行main方法中的代码，接着main方法中如果需要使用其他类，则继续加载

* 当jvm进程刚启动时，会把main方法作为入口，main方法中直接用到的类都会被加载到jvm中。
* 后面jvm运行过程中代码第一次用到某个类时，就需要从.class文件中把类加载到jvm中。是按需加载，而不是一次性都加载进jvm
...
* 验证:(先加载进内存后再验证)就是以 Java 虚拟机规范为标准，来校验加载进来的“.class”文件中的内容是否符合规范，主要是为了维护虚拟机自身的安全
* 准备(核心阶段):字节码文件加载进内存后，在准备阶段需要给类分配一定的内存空间，然后给类变量分配内存空间，并给类变量初始化为默认值。(给ReplicaManager.class类分配空间，给类变量flushInterval分配空间，然后把初始值0赋给类变量flushInterval)
>1. 给类分配内存空间
>2. 给类变量分配空间并赋默认值
* 解析: 把符号引用替换为直接引用
* 初始化(核心阶段): 
...
* 如果加载jdk中的类，webAppClassLoader会委派给应用程序类加载器走双亲委派模型去加载。如果加载的不是jdk中的类，webAppClassLoader自己先尝试加载，如果无法加载，就委派给shared类加载器走双亲委派模型去加载
...
# 线上JVM优化实战过程:
* 线上JVM配置:堆内存空间分配256MB，按JDK1.8默认分配，新生代和老年代比例为1:2,新生代分配85MB，老年代分配171MB，新生代中Eden区分配79MB，s0和s1各分配3MB，目前老年代已使用127MB
