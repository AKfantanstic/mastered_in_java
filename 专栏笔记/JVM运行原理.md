...
CMS默认启动的垃圾收集线程数量 = (cpu核数 + 3) / 4,由于cms有两个阶段是垃圾收集线程和工作线程并行运行的，所以当使用双核cpu时，意味着需要占用一个核心来进行垃圾回收，工作线程只有一个核心了。

cms的缺点：消耗cpu资源

#### 什么情况下会发生Concurrent Mode Failure?

由于jvm参数:"-XX:CMSInitiatingOccupancyFaction"默认值为92，所以cms在进行fullgc之前会在老年代预留8%的空闲内存，当 cms在第四阶段并发清理阶段对标记好的垃圾对象进行清理期间，如果这时minorGC后需要把一些新存活对象放入老年代，超过了老年代当前可用内存空间，就会发生Concurrent Mode Failure。也就是说cms一边回收，另一边把对象放入老年代导致内存不够用了，并发垃圾回收失败了。

这时就会启用serialOld垃圾回收器替代CMS，强行进行stop the world ，使用单线程重新执行fullGC过程，一次性将垃圾对象都回收掉，然后再恢复工作线程

#### 触发老年代Full GC的时机？

* 第一: 新生代全部对象大小大于老年代当前可用内存大小，如果没开启空间担保参数，会直接触发FullGC，所以一般空间担保参数都会打开
* 第二: 空间担保参数打开了，但老年代当前可用内存小于历次新生代Gc后进入老年代的平均对象大小,此时会提前fullGC
